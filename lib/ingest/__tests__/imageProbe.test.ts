...
// Minimal valid PNG: signature + IHDR with width/height. The rest of the file
// would be other chunks; we only parse the first 24 bytes so this is enough.
...
// precision (08) + height (2 bytes BE) + width (2 bytes BE) + remainder padding.
function jpegHeader(width: number, height: number): Uint8Array {
  // Minimum length 24 — the probe's outer guard rejects buffers below that.
